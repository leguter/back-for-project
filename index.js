...
// // У пам'яті сервера будемо зберігати дані користувачів
// const userStore = new Map();

// // --- ІМІТАЦІЯ БАЗИ ДАНИХ ПРЕДМЕТІВ ---
// const ITEMS_DB = {
...
// function authenticateToken(req, res, next) {
//     const authHeader = req.headers['authorization'];
//     const token = authHeader && authHeader.split(' ')[1]; // Отримуємо токен з заголовка 'Bearer TOKEN'

//     if (token == null) return res.sendStatus(401); // Якщо токену немає

//     jwt.verify(token, process.env.JWT_SECRET, (err, userPayload) => {
//         if (err) return res.sendStatus(403); // Якщо токен невалідний
        
//         const fullUser = userStore.get(userPayload.id);
//         if (!fullUser) return res.sendStatus(401); // Якщо користувача немає в нашій "базі"
        
//         req.user = fullUser; // Додаємо дані користувача до об'єкту запиту
//         next();
//     });
// }
...
//     // Розбираємо рядок initData, щоб отримати дані користувача
...
// // Захищені маршрути (профіль, кейси, інвентар)
// app.get('/api/profile', authenticateToken, (req, res) => {
//     res.status(200).json({ loggedIn: true, user: req.user });
// });
